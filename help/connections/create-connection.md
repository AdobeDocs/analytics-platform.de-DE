...
>[!IMPORTANT]
>
>Fügen Sie mindestens einen Ereignis- oder Zusammenfassungsdatensatz (Standard oder vom Typ Ad-hoc oder modellbasiert) zu Ihrer Verbindung hinzu.
...
Alternativ können die oben aufgeführten Datensatztypen auf einem Ad-hoc- oder modellbasierten Schema anstelle eines generischen XDM-basierten Schemas basieren.
...
| **[!UICONTROL Modell]** | Modellbasierte Daten basierend auf einem modellbasierten Schema. | Abhängig vom Datensatztyp, den Sie für den modellbasierten Datensatz auswählen. | Jedes modellbasierte Schema. | Abhängig vom Datensatztyp, den Sie für den modellbasierten Datensatz auswählen. |
...
1. Im ➊ Schritt **[!UICONTROL Datensätze auswählen]** wird eine Liste der Experience Platform-Datensätze angezeigt.
   ...
   | **[!UICONTROL Typ des Datensatzes]** | Der Typ des Datensatzes: [Ereignis](#event-dataset), [Profil](#profile-dataset), [Suche](#lookup-dataset), [Zusammenfassung](#summary-dataset), [Adhoc](#ad-hoc-dataset) oder [Modell](#model-based-dataset). |
   | **[!UICONTROL Anzahl der Einträge]** | Die Gesamtzahl der Einträge im Vormonat für den Datensatz in Experience Platform. |
...
1. Konfigurieren Sie [Datensatzeinstellungen](#dataset-settings) im Dialogfeld **[!UICONTROL Datensatz bearbeiten: _Datensatzname_]**.
   ...
   >[!NOTE]
   >
   >Sie können den **[!UICONTROL Datensatztyp]**, die **[!UICONTROL Personen-ID]**, den **[!UICONTROL Identity-Namespace]** und **[!UICONTROL Zeitstempel]** nicht für einen [Ad-hoc](#ad-hoc-dataset) oder einen [modellbasierten](#model-based-dataset) Datensatz bearbeiten, der Teil einer gespeicherten Verbindung ist. So ändern Sie eine dieser Einstellungen:
   >
   >1. Löschen Sie den vorhandenen Ad-hoc- oder modellbasierten Datensatz aus der Verbindung.
...
>[!NOTE]
>
>Obwohl es möglich ist, zu konfigurieren und auszuwählen, sollten Sie aus Leistungsgründen vermeiden, einen Ad-hoc-Datensatz für Zeitreihendaten (Ereignis-, Zusammenfassungsdaten) zu verwenden. Modellbasierte oder generische XDM-basierte Datensätze eignen sich viel besser für Zeitreihendaten als Ad-hoc-Datensätze.
...
| **[!UICONTROL Personen-ID]** | Ereignis, Profil | Wählen Sie ein Feld aus dem Ad-hoc- oder modellbasierten Schema aus, das die Personen-ID darstellt. Dieses Feld kann ein beliebiges Feld im Datensatz sein. Wählen Sie aus **[!UICONTROL Identity-Namespace]** Feldern oder aus **[!UICONTROL Nicht-Identitätsfeldern]**. <br/>Sie können eine Kennung nur dann aus **[!UICONTROL Identity-Namespace]** auswählen, wenn mindestens ein Feld im Ad-hoc-Schema als Identität gekennzeichnet ist und einen Identity-Namespace hat. |
...
#### Modellbasierter Datensatz

>[!NOTE]
>
>Modellbasierte Datensätze werden hauptsächlich zur Unterstützung der bevorstehenden Experience Platform Data Mirror for Customer Journey Analytics-Funktionen verwendet.
>

Die spezifischen Einstellungen für einen modellbasierten Datensatz sind:

| Einstellung | Ausgewählter Datensatztyp | Beschreibung |
|---|---|---|
| **[!UICONTROL Typ des Datensatzes]** | k. A. | Der Datentyp im modellbasierten Datensatz.<br/>Wenn der Datensatz Zeitreihendaten enthält, sind die möglichen Werte: **[!UICONTROL Ereignis]** und **[!UICONTROL Zusammenfassung]**. <br/>Wenn der Datensatz Datensatzdaten enthält, lauten die möglichen Werte: **[!UICONTROL Profile]** und **[!UICONTROL Lookup]**. |
| **[!UICONTROL Personen-ID]** | Ereignis, Profil | Wählen Sie ein Feld aus dem modellbasierten Schema aus, das die Personen-ID darstellt. Die Auswahl ist auf die Liste der Felder im modellbasierten Schema beschränkt, die als Identität markiert sind und keinen Identity-Namespace haben. |
...
Manchmal ist es erforderlich, Daten aus einem oder mehreren Datensätzen erneut in eine Verbindung aufzunehmen. Bei Ad-hoc- oder modellbasierten Datensätzen müssen Sie [löschen und dann den Datensatz erneut hinzufügen](#edit-a-dataset). Für andere Datensätze können Sie die Einstellungen aktualisieren. Gehen Sie dazu wie folgt vor:
