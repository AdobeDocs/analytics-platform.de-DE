...
| Funktion | Mehr Infos |
| --- | --- |
| Möglichkeit zur Kombination von Datensätzen (z. B. Report Suites von Adobe Analytics) | Mit Customer Journey Analytics können Sie [Daten kombinieren](/help/connections/combined-dataset.md) aus mehreren Report Suites, als ob es sich um eine einzige Report Suite in Adobe Analytics handelte. |
...
| Abgeleitete Felder | [Abgeleitete Felder](/help/data-views/derived-fields/derived-fields.md) ermöglichen Berichtszeittransformationen an Ihren Daten. Daten können dynamisch kombiniert, korrigiert oder erstellt werden und gelten rückwirkend für alle Berichte. |
| Verbesserte Sicherheits- und Datenschutzoptionen – HIPAA-Fähigkeit | Customer Journey Analytics ist HIPAA-bereit und bietet [Zusätzliche Sicherheitsoptionen](/help/privacy/cmk.md) zur Einhaltung der Vorschriften. Adobe Analytics ist nicht HIPAA-fähig. |
| Experimentanalyse | Customer Journey Analytics kann [Beurteilung der Steigerung und Konfidenz eines Experiments](/help/analysis-workspace/c-panels/experimentation.md) aus jeder Datenquelle, die als Teil einer Verbindung definiert wurde. Diese Auswertung ermöglicht Ihnen, die kausalen Zusammenhänge zwischen Kundeninteraktionen zu verstehen, die sich über alle Kanäle erstrecken. Analytics ist auf die Experimentanalyse über A4T beschränkt. |
| Prognose | [Prognosen](/help/analysis-workspace/c-forecast/forecasting.md) ist eine KI-/ML-Funktion, die eine statistische Prognose für zeitreihenbezogene Daten enthält, die auf den bereits in Customer Journey Analytics vorhandenen historischen Daten basieren. Prognosen können in Freiformtabellen und Liniendiagrammvisualisierungen angezeigt werden. |
| Geführte Analyse | [Geführte Analyse](/help/guided-analysis/overview.md) ist ein Berichtsformat, das es Benutzern ermöglicht, sich schnell selbst für ihre Datenanforderungen zu entscheiden, sodass sie schnell hochwertige Einblicke erhalten und datenbasierte Entscheidungen treffen können. „Geführte Analyse“ ist Bestandteil von Adobe Product Analytics, einem Add-on zu Customer Journey Analytics. |
...
| Umwandlungen zum Zeitpunkt der Berichtserstellung | [Datenansichten](/help/data-views/data-views.md) in Customer Journey Analytics können Sie Daten aus einer Verbindung weiter interpretieren. Sie können Daten ändern oder entfernen, ohne Ihre Implementierung zu ändern, Teilzeichenfolgen verwenden, um Dimensionen zu bearbeiten, Metriken aus beliebigen Werten erstellen oder Teilereignisse filtern. Alle diese Umwandlungen erfolgen zerstörungsfrei. Adobe Analytics bietet begrenzte Möglichkeiten durch Virtual Report Suites und benutzerdefinierte Sitzungslängen. |
| BI-Erweiterung | Die [BI-Erweiterung](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-usecases/data-export/bi-extension) ermöglicht Ihnen, CJA direkt mit gängigen BI-Visualisierungs-Tools wie PowerBI oder Tableau zu verbinden. Durch die Verwendung dieser Erweiterung können Sie Ihre BI-Berichte exakt mit denen in Analysis Workspace und anderen CJA-Reporting-Schnittstellen übereinstimmen. Dies ist eine wesentlich einfachere Möglichkeit, BI-Berichte für CJA zu erhalten, ohne dass Berichte/Metriken aus Rohdaten neu erstellt werden müssen. |
