...
>abstract="Fügen Sie Benutzer und Gruppen hinzu, die benachrichtigt werden sollen, wenn dieser Export fehlschlägt oder demnächst abläuft."
...
1. Geben Sie im Dialogfeld [!UICONTROL **Neuer vollständiger Tabellenexport**] die folgenden Informationen an:
   ...
   | Datenstruktur | Zeigt die Freiformtabelle an, die exportiert wird. Sie können die Datenstruktur ändern, indem Sie Komponenten aus dem linken Panel in die Tabelle ziehen. Sie können einen Filter anwenden, indem Sie eine Komponente in den Filterbereich ziehen. Die Tabelle wird beim Hinzufügen von Komponenten zur Arbeitsfläche dynamisch aktualisiert. Sie können bis zu 10 Spalten einschließen.<p>Alle Segmente, die auf die vollständige Tabelle im Projekt angewendet wurden, werden über der Tabelle angezeigt. Sie können ein Segment oder eine Gruppe von Segmenten auf einen Export anwenden.</p> |
