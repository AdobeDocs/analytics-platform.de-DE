...
Um Ihre Customer Journey Analytics-Nutzung zu ermitteln, können Sie verschiedene Methoden verwenden:

* Addieren Sie die Ereignisdatenzeilen für jede Verbindung. Siehe [Schätzen der Verbindungsgröße](#estimate-connection-size) unten. Dies ist eine einfache Methode, um Ihre Ereigniszeilendaten pro Verbindung für einen bestimmten Zeitstempel anzuzeigen.
...
So verwalten Sie die Customer Journey Analytics-Nutzung:
...
1. Addieren Sie die **[!UICONTROL verfügbaren Sätze von Ereignisdaten]** für jede Verbindung, die Ihr Unternehmen erstellt hat. (Je nach Größe Ihrer Verbindung kann es eine Weile dauern, bis die Anzahl angezeigt wird.)
   ...
   ![Datensätze der Ereignisdaten verfügbar.](./assets/event-data.png)

   >[!CAUTION]
   >
   >   Diese Anzahl beinhaltet nur Ereignisdaten, nicht aber Profil- oder Lookup-Daten. Wenn Sie über Profil- und Lookup-Daten verfügen, ist die Anzahl etwas höher. Es gibt jedoch derzeit keine Möglichkeit, Berichte zur Nutzung von Profil- und Lookup-Daten in der Benutzeroberfläche zu erstellen.
...
1. Um dieses Problem zu beheben, haben Sie mehrere Möglichkeiten:
   ...
   * Wenden Sie sich an Ihr Adobe-Account-Team, um zusätzliche Kapazität zu lizenzieren.
...
1. Erstellen Sie in Workspace neue Projekte auf der Grundlage der einzelnen Datenansichten und rufen Sie (aus der Dropdown-Liste **[!UICONTROL Metriken]**) alle Ereignisse bis zum ersten Freitag des Monats ab, beginnend mit dem ersten Tag Ihres aktuellen Customer Journey Analytics-Vertrags.
...
1. Verwenden Sie die [Customer Journey Analytics-Reporting](https://developer.adobe.com/cja-apis/docs/api/#tag/Reporting-API)API, um einen Bericht zu allen Ereignisdaten (**jede Verbindung)**. Richten Sie dies so ein, dass der Bericht folgendermaßen ausgeführt wird:

   * An jedem ersten Freitag eines jeden Monats
   * Zurück bis zum ersten Tag Ihres aktuellen Customer Journey Analytics-Vertrags.

   Dadurch erhalten Sie eine gute Vorstellung davon, wie Ihre Nutzung von Monat zu Monat aussieht. Dadurch erhalten Sie die Gesamtzahl der Zeilen für alle Ihre Customer Journey Analytics-Verbindungen.
...
Um Ihre Nutzung zu verwalten, können Sie [Verbindungs-Benutzeroberfläche](/help/connections/create-connection.md) auf Verbindungsebene die Customer Journey Analytics-Datenspeicherung als rollierendes Fenster in Monaten (1 Monat, 3 Monate, 6 Monate usw.) definieren.
